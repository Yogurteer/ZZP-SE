...
void test_KS2E(){
    ...
    outFile << "---|KS2E Protocol Test|---" << endl;
    ...
    for(auto w:select_queries) {
        ...
        //***client端search***//
        vector<vector<unsigned char>> request;
        c.search(w, request);
        //***分隔符***//

        //***server端search***//
        vector<vector<unsigned char>> response;
        s.search(client_type, request, response, db1);
        //***分隔符***//

        //***client端callback***//
        vector<vector<unsigned char>> result; // result={fileid1,fileid2...}
        vector<vector<unsigned char>> param = {w}; // 解码输入参数,一般为搜索关键字w,根据协议需要修改
        c.callback(param, response, result);
        //***分隔符***//
