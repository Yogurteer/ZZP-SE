...
void fullshare_w(KS2EUser &u, KS2EServer &s, vector<vector<unsigned char>> &Pw)
{
    ...
    // view u_share_id_result
    search_output(u_share_w_result, share_w_dir);
...
void fullsearch_w(string client_type, KS2EClient &c, KS2EServer &s, vector<unsigned char> w, vector<vector<unsigned char>>& getids)
{
    // user gen trapdoor for id
...
void batchsearch_id(string client_type, KS2EClient &c, KS2EServer &s, vector<vector<unsigned char>> IDq, 
    ...
    for(auto id:IDq) {
        fullsearch_id(client_type, c, s, id, getws);
        result[id] = getws;
        getws.clear();
    }
}
...
void batchsearch_w(string client_type, KS2EClient &c, KS2EServer &s, vector<vector<unsigned char>> Wq, 
    ...
    for(auto w:Wq) {
        fullsearch_w(client_type, c, s, w, getids);
        result[w] = getids;
        getids.clear();
    }
}
