...
void KS2EClient::update(vector<unsigned char> w, vector<unsigned char> id, vector<unsigned char> op,
    vector<vector<unsigned char>>& tokens){
    
    vector<vector<unsigned char>> Cwid;
    tokens.clear(); // 清空tokens
    
    pair<vector<unsigned char>, vector<unsigned char>> w_id = make_pair(w, id);
    if(this->st.W_ID.find(w_id)!=this->st.W_ID.end()){// escape existing w||id
        cout<<"w: " ;view_pt(w);cout<<" id: " ;
        printVector(id);cout<<endl;
        throw runtime_error("Continue: current (w,id) exist in client,not repeat update!");
        return;
    }
    ...
    // client封装L,I,R,C等密文索引发给server
    tokens = {L, Iid, Rid, Cid, Iw, Rw, Cw};
}
...
void KS2EClient::search(vector<unsigned char> w, vector<vector<unsigned char>>& request)
{
    ...
    request.clear();
    request.push_back(L);
    request.push_back(Jw);
}

void KS2EClient::search_token_id(vector<unsigned char> id, vector<vector<unsigned char>>& Tid)
{
    vector<unsigned char> k1 = this->k1;
    vector<unsigned char> k2 = this->k2;
...
void KS2EClient::callback(vector<vector<unsigned char>> param, vector<vector<unsigned char>> response, vector<vector<unsigned char>>& result)
{
    ...
    vector<unsigned char> w = param[0];
    vector<unsigned char> kw = gm_hmac(w.data(),w.size(),k2.data(),k2.size(),f_len);

    for(auto &Cw:response){
...
vector<unsigned char> KS2EClient::gen_Rw(size_t len) {
    ...
    //     Rw[i] = static_cast<unsigned char>(dist(gen));
    // }
    ...
    vector<vector<vector<unsigned char>>> &S_1, map<vector<unsigned char>, vector<vector<unsigned char>>>& result)
{
    ...
    for(auto &s:S){
        ...
        auto Rw = Cwid[5];
        auto Cw = Cwid[6];
        vector<vector<unsigned char>> row = {L, Iid, Rid, Cid, Iw, Rw, Cw};
        S_1.push_back(row);
    }
}
...
void KS2EServer::update(vector<vector<unsigned char>> tokens, DBOGaccess& db1)
{
    vector<vector<unsigned char>> Cwid = tokens;
    auto L = Cwid[0];
    auto Iid = Cwid[1];
    auto Rid = Cwid[2];
    auto Cid = Cwid[3];
...
void KS2EServer::re_share_w_2(vector<vector<vector<unsigned char>>> &S_1, DBOGaccess& db1)
{
    for(auto &row:S_1){
        auto L = row[0];
        auto Iid = row[1];
        ...
        auto Rw = row[5];
        auto Cw = row[6];
        std::vector<std::vector<uint8_t>> params = {L, Iid, Rid, Cid, Iw, Rw, Cw};
...
void KS2EServer::re_share_id_2(vector<vector<vector<unsigned char>>> &S_1, DBOGaccess& db1)
{
    for(auto &row:S_1){
        auto L = row[0];
        auto Iid = row[1];
...
void KS2EServer::search(string type, vector<vector<unsigned char>> request, vector<vector<unsigned char>>& response, DBOGaccess& db1)
{
    string table_name;
